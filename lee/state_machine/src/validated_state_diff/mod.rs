...
impl ValidatedStateDiff {
    ...
    fn execute_authorized(
        ...
        let declared: HashSet<AccountId> = positions
            .iter()
            .map(|position| position.account_id)
            .collect();
        // Which declared positions a program actually received. A balance-only position leaves
        // the account untouched, so presence in the diff cannot stand in for having been seen.
        let mut positions_seen: HashSet<Position> = HashSet::new();
        ...
        while let Some((chained_call, caller_data)) = chained_calls.pop_front() {
            ...
            // The caller only names positions; resolve each one's actual value from the
            ...
            let real_pre_states: Vec<Input> = chained_call
                .positions
                .iter()
                .map(|position| {
                    ...
                    Ok(Input::at(*position, is_authorized(&account_id), &account))
                })
            ...
            // A chained callee must account for exactly the positions its caller named, in
            ...
            for pre in program_output.state_diffs.iter().map(|diff| &diff.pre) {
                ...
                // state or with any modifications to those values during the chain of calls. Only
                // the named shard is compared — a shard the position never named is invisible to
                // the guest and so cannot be reported.
                let position = Position::from(pre);
                let expected = state_diff
                    .get(&account_id)
                    .cloned()
                    .unwrap_or_else(|| state.get_account_by_id(account_id));
                let expected_pre = Input::at(position, pre.is_authorized, &expected);
                ensure!(
                    expected_pre == *pre,
            ...
            // Splice each named position into its account: a shard no position named must
            ...
            // `chained_call.positions` (bare positions the caller supplied, carrying no
        ...
        // Every position the caller declared must have reached a program.
        for position in positions {
            ensure!(
                positions_seen.contains(position),
    ...
    pub fn from_privacy_preserving_transaction(
        ...
        // Rebuild what the circuit journalled per public account: `pre` is this chain's own
        // value restricted to the namespaces the action touched, so a namespace the transaction
        // never named is neither anchored nor written.
        let public_actions: Vec<PublicAction> = message
            ...
            .map(|action| PublicAction {
                ...
                pre: state
                    .get_account_by_id(action.account_id)
