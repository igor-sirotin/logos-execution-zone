...
impl ExecutionState {
    ...
    fn journal_first_sight(
        ...
        if let Some(witness) = witness {
            ...
            let granted = seed_granted(caller, caller_pda_seeds, account_id);
            match granted {
                Some((program, seed)) => {
                    assert!(
                        pre.is_authorized,
    ...
    fn check_known_account(
        ...
        let granted = witness.map_or_else(
            || seed_granted(caller, caller_pda_seeds, account_id),
...
// A private PDA's address was derived from the `(program, seed)` binding its own witness carries,
// and one witness backs one address (`witness_by_account`), so the grant is a comparison against
// that binding rather than a re-derivation per candidate seed.
...
/// public address can be authorized; the private form compares the witness binding instead.
fn seed_granted(
    caller: &CallerData,
    caller_pda_seeds: &[PdaSeed],
...
mod tests {
    ...
    // Only `kind` is read by the grant; the rest is filler a witness cannot be built without.
